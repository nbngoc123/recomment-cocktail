...
from conected_psql import generate_embeddings, DatabaseSetup
load_dotenv()
...
class CocktailDataProcessor:
    ...
    def store_cocktails(self, df):
        ...
        try:
            ...
            batch_size = 32  # Process in batches to avoid API limits
            
            for i in range(0, len(df), batch_size):
                current_batch_num = i // batch_size + 1
                if current_batch_num == 4:
                    print("Skipping batch 4 as requested.")
                    continue
                
                batch_df = df.iloc[i:i+batch_size]
                print(f"Processing batch {current_batch_num}/{(len(df) + batch_size - 1)//batch_size}...")

                # Generate embeddings for the current batch
                combined_texts_batch = batch_df['combined_text'].tolist()
                # Ensure no empty strings are sent to the API
                sanitized_batch = [text if text.strip() else " " for text in combined_texts_batch]
                embeddings_batch = self.generate_embeddings(sanitized_batch, "BAAI/bge-large-en-v1.5")
                ...
                for batch_idx, (df_idx, row) in enumerate(batch_df.iterrows()):
                    ...
                    """, (name, ingredients, recipe, glass, category, iba, alcoholic, embedding_str))
                
                conn.commit() # Commit after each batch
                print(f"Stored {i + len(batch_df)} of {len(df)} cocktails...")
